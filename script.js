    ...
    try {
        ...
        productsList = rows.map(row => {
            ...
            return {
                ...
                // 🎯 التعديل الرئيسي: إزالة مجلد images/ لأن الصور مرفوعة في الجذر مباشرة
                img: `${imgValue}.png` 
            };
        });
...
// ⚠️ إرسال كلمات البحث، اسم المستخدم، ونوع الجهاز إلى شيت جوجل تلقائياً
searchInput.addEventListener('input', (e) => {
    ...
    if (query.length >= 2) {
        // الانتظار لمدة ثانيتين بعد توقف المستخدم عن الكتابة لتسجيل الكلمة كاملة
        searchTimeout = setTimeout(() => {
            ...
            fetch(appsScriptUrl, {
                method: 'POST',
                mode: 'no-cors', // لتفادي قيود الـ CORS بالمتصفح
                headers: {
